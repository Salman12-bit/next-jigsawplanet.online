...
function generatePath({ width, height, top, right, bottom, left }) {
    const tabSize = Math.min(width, height) / 5;
    let path = "M 0 0";
...
export default function jigsawBoard() {
    const piecesImage = "/images/jigsawbox.webp";
    ...
    useEffect(() => {
        async function processImage() {
            try {
                const img = await loadImage(piecesImage);
    ...
    useEffect(() => {
        const savedData = JSON.parse(localStorage.getItem("puzzle20")) || [];
    ...
    const updateLeaderboard = (name, time) => {
        ...
        const currentLeaderboard = JSON.parse(localStorage.getItem("puzzle20")) || [];
        ...
        localStorage.setItem("puzzle20", JSON.stringify(top7));
        setLeaderboard(top7);
    };
    ...
    return (
        <div className={styles.container}>
            <div className={styles.sidebar}>
                <h1 className={styles.centerText}>Jigsaw Puzzle Board – Match, Solve, and Have Fun Online</h1>
                <div className={styles.imageContainer}>
                    <img
                        src={piecesImage}
                        alt="Online jigsaw puzzle board with colorful interlocking pieces"
                        style={{
                            height: "200px",
                            width: "200px",
                            borderRadius: "10px",
